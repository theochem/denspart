...
def prepare_input_gpw(atoms, calc, grid_size):
    ...
    compute_augmentation_spheres(
        pseudo_grid_data, setups, atoms, atnums, atcoords, grid_size, cellvecs
    )
...
def compute_augmentation_spheres(
    grid_data, setups, atoms, numbers, coordinates, grid_size, cellvecs
):
...
def eval_correction(grid, atomg, setupg, center):
    ...
    if True:
        ...
        for ibasis0 in range(len(basis_fns)):
            for ibasis1 in range(ibasis0 + 1):
                olp[ibasis0, ibasis1] = grid.integrate(
                    basis_fns[ibasis0] * basis_fns[ibasis1]
                )
                olp[ibasis1, ibasis0] = olp[ibasis0, ibasis1]
                olpt[ibasis0, ibasis1] = grid.integrate(
                    basist_fns[ibasis0] * basist_fns[ibasis1]
                )
    ...
    density_v, density_vt, spindensity_v, spindensity_vt = (
        np.zeros(N),
        np.zeros(N),
        np.zeros(N),
        np.zeros(N),
    ...
    for ibasis0 in range(len(basis_fns)):
        for ibasis1 in range(ibasis0 + 1):
            ...
            density_v += (
                factor * dm[ibasis0, ibasis1] * basis_fns[ibasis0] * basis_fns[ibasis1]
            )
            density_vt += (
                factor
                * dm[ibasis0, ibasis1]
...
def _get_pseudo_grid_data(calc, cellvecs, atnums):
    ...
    for i in range(3):
        assert np.allclose(calc.density.gd.h_cv[i], cellvecs[i] / grid_data["shape"][i])
    ...
    if calc.wfs.nspins == 1:
        ...
        grid_data["ae_density"] = calc.get_all_electron_density(gridrefinement=1) * (
            Bohr ** 3
        )
    else:
...
def denspart_conventions(grid_data, atoms, atnums):
    ...
    for iatom in range(natom):
        grid_parts.append(
            GridPart(atoms["%03i" % iatom], ["density_c_cor", "density_v_cor"])
        )
